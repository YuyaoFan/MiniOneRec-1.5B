...
class ReReTrainer(Trainer):
    ...
    def _prepare_inputs(self, inputs: dict[str, Union[torch.Tensor, Any]]) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        # Compute summed rewards for logging/metrics (used in both branches)
        rewards = (rewards_per_func * self.reward_weights.to(device).unsqueeze(0)).sum(dim=1)
        # Compute group-wise std for logging (used in both branches)
        std_grouped_rewards = rewards.view(-1, self.num_generations).std(dim=1)
        std_grouped_rewards = std_grouped_rewards.repeat_interleave(self.num_generations, dim=0)

        if self.gdpo and len(self.reward_funcs) > 1:
            # GDPO: per-reward decoupled group normalization + batch normalization
