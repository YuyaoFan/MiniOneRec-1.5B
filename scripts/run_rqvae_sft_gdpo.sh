#!/bin/bash
# =============================================================================
# RQ-VAE + SFT + GDPO (Baseline — 最优方案)
# =============================================================================
set -e
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
cd "$ROOT"
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
LOGDIR="$ROOT/logs/rqvae_gdpo_${TIMESTAMP}"
mkdir -p "$LOGDIR"
...
# ── Stage 3: RL GDPO ──
RL_OUT="$ROOT/output/rl/${CATEGORY}_rqvae_gdpo"
...
HF_ENDPOINT=https://hf-mirror.com accelerate launch --config_file ./config/zero2_opt.yaml --num_processes 1 --main_process_port 29504 rl.py --model_path "$SFT_OUT" --train_batch_size 32 --eval_batch_size 32 --num_train_epochs 2 --gradient_accumulation_steps 32 --train_file "$TRAIN_CSV" --eval_file "$EVAL_CSV" --info_file "$INFO" --category "$CATEGORY" --sample_train False --eval_step 0.0999 --reward_type ranking --num_generations 16 --sync_ref_model True --beam_search True --temperature 1.0 --learning_rate 1e-5 --beta 1e-3 --gdpo True --output_dir "$RL_OUT" --sid_index_path "./data/Amazon/index/${CATEGORY}.index.json" --item_meta_path "./data/Amazon/index/${CATEGORY}.item.json" 2>&1 | tee "$LOGDIR/s3_rl.log"

# ── Stage 4: Eval ──
python tools/eval_all.py --batch_size 8 2>&1 | tee "$LOGDIR/s4_eval.log"
echo "[DONE] RQ-VAE+SFT+GDPO complete"
