"""
Trainer for RQ-MoE model in MiniOneRec.

Adapted from rq/trainer.py. Uses Normalized Residual Loss (NRL)
instead of standard MSE + commitment loss for improved deep-layer
gradient flow and training stability.

TensorBoard logging is automatically enabled and writes to
logs/tensorboard/stage1/ for real-time training visualization.
"""
...
try:
    from tools.tb_logger import TBStage1Logger
    _TB_AVAILABLE = True
except ImportError:
    _TB_AVAILABLE = False
...
class RQMoETrainer(object):
    """Trainer for RQ-MoE codebook generation.

    Args:
        args: training arguments (argparse.Namespace)
        model: RQMoEWrapper model
        data_num: number of batches per epoch (for scheduler)
    """
    ...
    def __init__(self, args, model, data_num):
        ...
        self.tb_logger = None
        if _TB_AVAILABLE:
            tb_log_dir = os.path.join("logs", "tensorboard", "stage1", saved_model_dir)
            try:
                self.tb_logger = TBStage1Logger(tb_log_dir)
    ...
    def _build_optimizer(self):
        ...
        if learner.lower() == "adam":
            ...
            self.logger.warning(f"Unknown optimizer '{learner}', using Adam")
    ...
    def _save_checkpoint(self, epoch, collision_rate=1, ckpt_file=None):
        ckpt_path = os.path.join(self.ckpt_dir, ckpt_file) if ckpt_file \
            else os.path.join(self.ckpt_dir,
        ...
        for epoch_idx in range(self.epochs):
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                if self.tb_logger is not None:
                    ...
                    self.tb_logger.log_eval(collision_rate, epoch_idx)
